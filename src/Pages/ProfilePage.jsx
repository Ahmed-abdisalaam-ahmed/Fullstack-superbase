...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
        ...
        if(avatar){
          ...
          const {error: avatarError} = await supabase.storage.from("avatars").upload(filePath, avatar);
  ...
  const handleAvatarChange = (e) => {
    if(e.target.files && e.target.files[0]){
      ...
      if(file.size > 2 * 1024 * 1024){
        toast.error("hey! the file is too large", {position: "top-right"})
        return
      }else{
        toast.success("the size is perfect 👍", {position: "top-center"})
      }
      setAvatar(file)
  ...
  return (
    <div className="max-h-screen bg-gray-50 py-32 px-4 sm:px-6 lg:px-8">
